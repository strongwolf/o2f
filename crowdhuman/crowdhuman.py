...
from .crowdhuman_metric import _evaluate_predictions_on_crowdhuman
...
class CrowdHumanDataset(CustomDataset):
    ...
    def format_results(self, results, jsonfile_prefix=None, **kwargs):
        ...
        if jsonfile_prefix is None:
            ...
            jsonfile_prefix = osp.join(tmp_dir.name)
        else:
            tmp_dir = None
        dt_path = osp.join(jsonfile_prefix, 'det.odgt')
        new_json_results = dict()
        for idx in range(len(self)):
            img_id = self.img_ids[idx]
            new_json_results[img_id] = dict()
            new_json_results[img_id]['ID'] = img_id
            new_json_results[img_id]['dtboxes'] = list()
            for k in range(len(results[idx][0])):
                res = results[idx][0][k]
                x,y,w,h = [float(item) for item in self.xyxy2xywh(res[:-1])]
                new_json_results[img_id]['dtboxes'].append({'box':[x,y,w,h], 'score':float(res[-1])})
        ...
        #return result_files, dt_path, tmp_dir
        return dt_path, tmp_dir
    ...
    def evaluate(self,
        ...
        dt_path, tmp_dir = self.format_results(results, jsonfile_prefix)
        eval_results = dict()
        '''
        eval_results = self.evaluate_det_segm(results, result_files, coco_gt,
                                              ...
                                              metric_items)
        '''
        gt_path = osp.join(osp.dirname(dt_path), 'gt.odgt')
        json_results = json.load(open(self.ann_file, "r"))
        img_dict = dict()
        ...
        ap = _evaluate_predictions_on_crowdhuman(gt_path, dt_path)

        for i, metric in enumerate(["AP", "mMR", "Recall"]):
            print(metric,":",ap[i])
        
        eval_results['AP_mMR_Recall'] = (
